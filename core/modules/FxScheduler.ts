...
export default class FxScheduler {
    ...
    constructor() {
        ...
        setInterval(() => {
            ...
            this.pushHudCountdown();
        }, 60 * 1000);
    }


    /**
     * Pushes the next-restart countdown to the in-game HUD (TxAdminPanel plugin).
     */
    private pushHudCountdown() {
        try {
            if (!txCore.rustRcon.isConnected) return;
            const isSkipped = this.nextSkip === this.calculatedNextRestartMinuteFloorTs;
            const arg = (this.calculatedNextRestartMinuteFloorTs && !isSkipped)
                ? String(this.calculatedNextRestartMinuteFloorTs)
                : 'off';
            txCore.rustRcon.sendCommand(`txadminpanel.setrestart ${arg}`, 5000).catch(() => { });
