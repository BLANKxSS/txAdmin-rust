...
import { ConfigSchemas_v2, PartialTxConfigs } from "./index";
...
const restructureOldConfig = (old: any) => {
    ...
    //NOTE: keys not present in the old config (eg. the new Rust server.* keys)
    // are omitted here, so they fall back to the schema defaults.
    const remapped: PartialTxConfigs = {
