...
export default class AdminStore {
    ...
    async loadAdminsFile() {
        ...
        jsonData.forEach((admin) => {
            ...
            //Remove FiveM-only permissions that don't apply to Rust servers
            const removedPerms = ['menu.vehicle', 'menu.clear_area', 'menu.viewids', 'players.freeze', 'players.heal', 'players.playermode', 'players.spectate', 'players.teleport', 'players.troll'];
            const beforeCount = admin.permissions.length;
            admin.permissions = admin.permissions.filter((perm) => !removedPerms.includes(perm));
            if (admin.permissions.length < beforeCount) {
                hasMigration = true;
            }
        });
