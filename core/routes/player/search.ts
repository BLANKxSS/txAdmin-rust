    ...
    */
    ...
    if (typeof searchType === 'string') {
        ...
        if (searchType === 'playerName') {
            ...
            if (invalids.length) {
                return sendTypedResp({ error: `Invalid identifiers (${invalids.join(',')}). Prefix any identifier with their type, like 'steam:1234567890abcdef' instead of just '1234567890abcdef'.` });
            }
