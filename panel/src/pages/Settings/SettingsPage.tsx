...
//Tab configuration
//NOTE: the 'FXServer' name is kept for the card/tab id ('fxserver') that the backend expects,
// but it is displayed as 'Server' (see TabsTrigger below).
type SettingsTabBase =
    | { name: string; Component: React.FC<SettingsCardProps> }
    | { name: string; cards: { name: string; Component: React.FC<SettingsCardProps> }[] };
const settingsTabsBase: SettingsTabBase[] = [
...
//Display name overrides (id stays stable for the backend API)
const tabDisplayName = (name: string) => {
    if (name === 'Whitelist') return 'Allowlist'; // FIXME:NEXT:UPDATE remove
    if (name === 'FXServer') return 'Server';
    return name;
}
                ...
                >
                    ...
                    >
                        {settingsTabs.map((tab) => (
                            ...
                            >
                                {tabDisplayName(tab.ctx.tabName)}
