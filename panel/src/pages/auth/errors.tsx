...
export function AuthError({ error }: AuthErrorProps) {
    ...
    if ('errorTitle' in error) {
        ...
        titleNode = 'Connection to login service timed out.';
        bodyNode = <ErrorText>
        ...
        bodyNode = <ErrorText>
            The login process was aborted. <br />
            Return to the login page to try again.
        </ErrorText>
    } else if (error.errorCode === 'end_user_logout') {
        titleNode = 'Login Aborted';
        bodyNode = <ErrorText>
            The login process was aborted because you logged out of your account. <br />
            Return to the login page to try again.
        </ErrorText>
        ...
        const accountId = error.errorContext?.identifier ?? 'unknown';
        const accountName = error.errorContext?.name ?? 'unknown';
        titleNode = `The account '${accountName}' is not an admin.`;
        bodyNode = <ErrorText>
            The account above with identifier <InlineCode>{accountId}</InlineCode> is not assigned to any account registered on txAdmin. <br />
