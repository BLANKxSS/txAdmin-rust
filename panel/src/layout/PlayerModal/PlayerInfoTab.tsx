...
export default function PlayerInfoTab({ playerRef, player, serverTime, tsFetch, setSelectedTab, refreshModalData }: PlayerInfoTabProps) {
    ...
    const [playerGroups, setPlayerGroups] = useState<string[] | null>(null);
    const [loadingGroups, setLoadingGroups] = useState(false);

    const getUserGroupsApi = useBackendApi<{ groups: string[] } | { error: string }>({
        method: "POST",
        path: "/oxideGroups",
        throwGenericErrors: false,
    });

    // Load player groups on mount
    useEffect(() => {
        const loadGroups = async () => {
            // Extract SteamID64 from player IDs
            const steamHex = player.idsOnline?.[0] || player.idsOffline?.[0];
            if (!steamHex || !steamHex.startsWith("steam:")) {
                setPlayerGroups([]);
                return;
            }

            const steamId64 = BigInt("0x" + steamHex.slice(6)).toString();

            setLoadingGroups(true);
            try {
                const result = await getUserGroupsApi({
                    data: {
    ...
    return (
        <ModalTabInner>
            ...
            <dl className="pb-2">
                ...
                <div className="py-0.5 grid grid-cols-3 gap-4 px-0">
                    <dt className="text-sm font-medium leading-6 text-muted-foreground">Groups</dt>
                    <dd className="text-sm leading-6 mt-0 col-span-2 flex flex-wrap gap-2">
                        {loadingGroups ? (
