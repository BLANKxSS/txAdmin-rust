...
export default function WarningBar() {
    const { offlineWarning, txUpdateData, serverUpdateData } = useWarningBar();
    ...
    if (offlineWarning) {
        return <InnerWarningBar
            ...
            description={<>
                ...
                If you closed txAdmin, please restart it.
            </>}
            isImportant={true}
            canPostpone={false}
        />
        ...
        return <InnerWarningBar
            titleIcon={<DownloadCloudIcon className="inline h-[1.2rem] -mt-1 mr-1" />}
            title={serverUpdateData.isImportant
                ? 'This version of the Rust server is outdated.'
                : 'An update is available for the Rust server.'}
            description={`Please update the Rust server to version ${serverUpdateData.version}.`}
            isImportant={serverUpdateData.isImportant}
            canPostpone={true}
        />
    } else {
        return null;
